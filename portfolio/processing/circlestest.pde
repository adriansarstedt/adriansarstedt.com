...
class PointSet {
  ...
  PointSet(int tn) {
    ...
    for (int x=0; x<n; x++) {
      ...
      vectors[x][0] = 5-random(10); vectors[x][1] = 5-random(10);
    }
  }
  ...
  void renderConnections(int p) {
    ...
    noFill();
    ellipse(points[p].x, points[p].y, min1[1], min1[1]);
    ellipse(points[p].x, points[p].y, min2[1], min2[1]);
    ellipse(points[p].x, points[p].y, min3[1], min3[1]);
